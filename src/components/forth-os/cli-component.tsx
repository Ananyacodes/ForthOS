...
export function CLIComponent({ history, onCommand }: CLIComponentProps) {
    ...
    <Card className="flex flex-col flex-1 h-full shadow-lg bg-card">
      ...
      <CardContent className="flex flex-col flex-1 p-0 overflow-hidden">
        <ScrollArea className="flex-1 p-3" ref={scrollAreaRef}>
          {history.map((item) => (
            <CLIOutput key={item.id} output={item} />
          ))}
        </ScrollArea>
        <form onSubmit={handleSubmit} className="flex items-center p-3 border-t">
          <span className="text-primary mr-2 select-none">ForthOS&gt;</span>
          <Input
            ref={inputRef}
            type="text"
            value={inputValue}
